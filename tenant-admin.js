...
(function (global) {
  ...
  function rebuildSelect(selected) {
    var sel = document.getElementById("tc-sel");
    if (!sel) return;
    /* Vergrendeld op één tenant (gewone gebruiker/tenant-admin)?
       Toon dan enkel de eigen tenant — geen wissel, geen "+ Nieuwe". */
    if (TC.locked && !global._isSuper) {
      sel.innerHTML = '<option value="' + esc(TC.tenant) + '" selected>' + esc(TC.tenant) + "</option>";
      sel.disabled = true;
      return;
    }
    sel.disabled = false;
  ...
  async function save() {
    var cfg = collect();
    /* Vergrendeld: branding aanpassen mag, maar niet onder een andere slug —
       de RLS in Supabase zou dat sowieso weigeren; hier vangen we het netjes af. */
    if (TC.locked && !global._isSuper && cfg.slug !== TC.tenant) {
      toast("⚠ Je kan enkel je eigen omgeving (" + TC.tenant + ") aanpassen.");
      return;
    }
    TC.register(cfg.slug, cfg);
    TC.tenant = cfg.slug;
  ...
  global.openTenantConfig = function () {
    ...
    rebuildSelect(TC.tenant);   /* past vergrendeling toe (locked → alleen eigen tenant) */
    fillForm();
