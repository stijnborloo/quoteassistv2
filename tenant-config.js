...
(function (global) {
  ...
  /* Deployment-standaard: welke tenant deze build toont wanneer er geen
     ?tenant= of lokale voorkeur is. Zet dit op de slug van je klant voor een
     één-tenant-deployment (bv. "orbid"). Leeg ("") = multi-tenant modus,
     waarbij subdomein → "default" de terugval is. */
  var DEFAULT_TENANT = "orbid";

  function resolveTenantId() {
    /* 1. expliciete override in de URL (ook DB-only tenants toestaan) */
    try {
      var q = new URLSearchParams(global.location.search).get("tenant");
      if (q) return String(q).toLowerCase();
    } catch (e) {}
    /* 2. door de app/admin gezette voorkeur (dit toestel) */
    try {
      var ls = global.localStorage.getItem("qs_tenant");
      if (ls) return ls;
    } catch (e) {}
    /* 3. deployment-standaard (geldt op elk toestel, ook zonder localStorage) */
    if (DEFAULT_TENANT) return DEFAULT_TENANT;
    /* 4. multi-tenant: subdomein, anders default */
    try {
