...
(function (global) {
  ...
  /* Laat elk logo (SVG of <img>) netjes meeschalen met zijn container.
     Verwijdert vaste width/height op de SVG en dwingt height:100% + width:auto
     af, zodat het logo overal (cover 34px, paginakop 22-30px) automatisch past
     zonder handmatig prutsen. */
  function fitLogo(html) {
    ...
    if (/<svg\b/i.test(inner)) {
      inner = inner.replace(/<svg\b([^>]*)>/i, function (m, attrs) {
        ...
        return '<svg' + attrs + ' style="height:100%;width:auto;max-width:100%;display:block">';
      });
    } else if (/<img\b/i.test(inner)) {
      inner = inner.replace(/<img\b([^>]*)>/i, function (m, attrs) {
        attrs = attrs.replace(/\sstyle\s*=\s*("[^"]*"|'[^']*')/i, "");
        return '<img' + attrs + ' style="height:100%;width:auto;max-width:100%;object-fit:contain;display:block">';
      });
    }
    return '<span style="display:inline-flex;align-items:center;height:100%;max-width:100%;line-height:0">' + inner + '</span>';
  }
  ...
  var TC = {
    ...
    logoPdf: function () {
      var t = TC.all();
      if (t.logo) return fitLogo(t.logo);
    ...
    logoWhitePdf: function () {
      var t = TC.all();
      if (t.logoWhite) return fitLogo(t.logoWhite);
      return '<span style="font-family:Inter,Arial,sans-serif;font-weight:800;'
    ...
    /* ── Async: laad ALLE tenants uit Supabase in het register ──
       Zodat de tenant-keuzelijst elke tenant toont, ongeacht toestel/
       localStorage. Verandert de actieve tenant niet. */
