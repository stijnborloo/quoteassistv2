...
(function (global) {
  ...
  /* Render een logo (SVG of <img>) op een expliciete hoogte in px met
     proportionele breedte. Expliciete px is nodig omdat de PDF-generator
     (html2canvas) preserveAspectRatio negeert en een SVG zonder afmetingen
     uitrekt. 'h' = doelhoogte in px. */
  function fitLogo(html, h) {
    ...
    if (/<svg\b/i.test(inner)) {
      inner = inner.replace(/<svg\b([^>]*)>/i, function (m, attrs) {
        /* aspect-ratio bepalen vóór het strippen: eerst viewBox, dan w/h */
        var aspect = 0;
        var vb = attrs.match(/viewBox\s*=\s*["']\s*[-\d.]+\s+[-\d.]+\s+([\d.]+)\s+([\d.]+)/i);
        if (vb) { aspect = parseFloat(vb[1]) / parseFloat(vb[2]); }
        if (!aspect) {
          var w = attrs.match(/\bwidth\s*=\s*["']?([\d.]+)/i);
          var hh = attrs.match(/\bheight\s*=\s*["']?([\d.]+)/i);
          if (w && hh) aspect = parseFloat(w[1]) / parseFloat(hh[1]);
        }
        if (!aspect || !isFinite(aspect)) aspect = 3;   /* redelijke terugval */
