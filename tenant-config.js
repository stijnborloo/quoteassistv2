...
(function (global) {
  ...
  var TENANTS = {
    ...
    "default": {
      ...
      /* Sectorterminologie: wat heet een "zaal" en een "product" in deze sector?
         AV → Zaal/Zalen/Product/Producten (baseline = HTML)
         Solar → Installatie/Installaties/Paneel/Panelen
         Security → Zone/Zones/Component/Componenten  enz.
         Vervanging loopt via TC.brandPairs() → brandSweep(), zelfde
         mechanisme als de merktekst-vervanging. */
      roomLabel:        "Zaal",
      roomLabelPlural:  "Zalen",
      productLabel:     "Product",
      productLabelPlural: "Producten",
      ...
      trialEndsAt:      null,          /* ISO timestamp, null = geen trial */
  ...
  var TC = {
    ...
    /* De termen die overal in zichtbare tekst vervangen worden.
       ...
       Meervoud vóór enkelvoud zodat "Zalen" niet al deels wordt
       vervangen door de "Zaal"-regel. */
    brandPairs: function () {
      var t = TC.all();
      var pairs = [];
      ...
      /* Sectorterminologie — meervoud eerst (langste match wint) */
      var rp = (t.roomLabelPlural || "").trim();
      if (rp && rp !== "Zalen") {
        pairs.push(["Zalen", rp]);
        pairs.push(["zalen", rp.toLowerCase()]);
      }
      var rs = (t.roomLabel || "").trim();
      if (rs && rs !== "Zaal") {
        pairs.push(["Zaal", rs]);
        pairs.push(["zaal", rs.toLowerCase()]);
      }
      var pp = (t.productLabelPlural || "").trim();
      if (pp && pp !== "Producten") {
        pairs.push(["Producten", pp]);
        pairs.push(["producten", pp.toLowerCase()]);
      }
      var ps = (t.productLabel || "").trim();
      if (ps && ps !== "Product") {
        pairs.push(["Product", ps]);
        pairs.push(["product", ps.toLowerCase()]);
      }
  ...
  function mapRow(r) {
    ...
    var map = {
      ...
      room_label: "roomLabel", room_label_plural: "roomLabelPlural",
      product_label: "productLabel", product_label_plural: "productLabelPlural"
    };
    ...
    if (r.trial_ends_at)  out.trialEndsAt  = r.trial_ends_at;
