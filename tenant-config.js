...
(function (global) {
  ...
  var TC = {
    ...
    /* Wissel actieve tenant en herbrand de UI.
       Bij een ECHTE wissel: de openstaande offerte + autosave horen bij de
       vórige tenant. Die laten staan betekent dat een volgende "Opslaan" hem
       naar de verkeerde tenant schrijft. Dus: wissen en herladen — dan bouwt
       supaInit() ook meteen een client met de juiste x-tenant-id header. */
    set: function (id) {
      if (!TENANTS[id]) { console.warn("[TC] onbekende tenant:", id); return; }
      var changed = (TC.tenant !== id);
      TC.tenant = id;
      try { global.localStorage.setItem("qs_tenant", id); } catch (e) {}
      if (changed) {
        try {
          // Werkstate van de vorige tenant weg. Bewust NIET alles wissen:
          // login (qs_auth_v1) en Supabase-config moeten blijven.
          // Sleutels komen uit index.html regel 6078 + 7032.
          ["qs_v3",        /* STOR — de openstaande offerte */
           "qs_offertes_v2", "qs_lib_v1", "qs_fotos_v1",
           "qs_items_v1",  "qs_zaaltpl_v1"]
