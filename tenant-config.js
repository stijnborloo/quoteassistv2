...
(function (global) {
  ...
  var TC = {
    ...
    /* Vergrendeld op de tenant uit de JWT? Dan kan alleen unlock() (super)
       nog van tenant wisselen. ?tenant= of localStorage omzeilt dit niet:
       de échte databeveiliging zit in Supabase RLS — dit is de UI-kant. */
    locked: false,

    /* Vergrendel de app op de tenant uit app_metadata (na login).
       Onbekende tenants worden minimaal geregistreerd; TC.load() haalt
       daarna de volledige branding uit Supabase op. */
    ...
    /* Alleen voor super-admins: heft de vergrendeling op zodat de
       tenant-keuzelijst in het configuratiescherm weer werkt. */
    unlock: function () { TC.locked = false; },

    /* Wissel actieve tenant en herbrand de UI */
    set: function (id) {
      if (TC.locked && id !== TC.tenant) {
        console.warn("[TC] tenant is vergrendeld op", TC.tenant, "— wissel genegeerd.");
        return;
      }
