...
(function (global) {
  ...
  async function loadUsers() {
    try {
      ...
      // Fallback: probeer via Supabase als worker niet werkt
      _users = [];
  ...
  async function renderList() {
    ...
    if (!panel) return;
    panel.innerHTML = '<div class="sa-empty"><span class="spin spin-r"></span> Laden…</div>';
  ...
  global.saCreateTenant = async function () {
    ...
    if (!name) {
      status.innerHTML = '<div class="sa-err">Bedrijfsnaam is verplicht.</div>';
      return;
    }
    ...
    var steps = [];
    try {
      // Stap 1: Tenant aanmaken in qs_tenants
      var c = supa();
      ...
      if (email && email.includes("@")) {
        try {
          ...
          // De Worker stempelt tenant_id + role='user'; we upgraden naar admin
      ...
      TC.register(slug, { slug: slug, companyName: name, companyNameShort: name, primaryColor: color });

      status.innerHTML = '<div class="sa-ok">' + steps.join("<br>") + '</div>';
      toast("✓ Klant " + name + " aangemaakt");
  ...
  global.saOnboard = function (slug) {
    ...
    var panel = document.getElementById("sa-panel-add");
    // Pre-fill slug en disable het veld
    setTimeout(function () {
  ...
  global.saAddMember = async function () {
    ...
    try {
      ...
      // Update ook de app_metadata als dit de primaire tenant is
  ...
  function injectMenuItem() {
    ...
    // Voeg in na de bestaande admin-items
  ...
  function ensureEntry() {
    // Alleen tonen voor super-admins
